...
class SpatioTemporalSplit(BaseCrossValidator):
    ...
    A custom cross-validator that splits the data along two dimensions:
      - Temporal: a cutoff date is determined so that training data includes only 
        observations with dates before that cutoff.
      - Spatial: the unique stations are randomly split so that training data includes 
        only a fraction (e.g. 75%) of the stations while the remaining stations appear in test.
    
    Parameters:
      n_splits: number of folds.
      date_col: column name for the observation date.
      station_col: column name for the station code.
      temporal_frac: fraction (between 0 and 1) of the timeline to use for training.
                     The cutoff date is chosen as the quantile of the unique dates.
      spatial_frac: fraction (between 0 and 1) of stations to include in training.
      random_state: random seed for reproducibility.
    """
    def __init__(self, n_splits=10, date_col='ObsDate', station_col='station_code', 
                 temporal_frac=0.75, spatial_frac=0.75, random_state=None):
    ...
    def split(self, X, y=None, groups=None):
        ...
        for i in range(self.n_splits):
            ...
            n_train_stations = int(len(unique_stations) * self.spatial_frac)
            if n_train_stations < 1 or (len(unique_stations) - n_train_stations) < 1:
                raise ValueError("The provided spatial_frac does not leave stations for both training and testing.")
            
            train_stations = stations_shuffled[:n_train_stations]
            test_stations = stations_shuffled[n_train_stations:]
